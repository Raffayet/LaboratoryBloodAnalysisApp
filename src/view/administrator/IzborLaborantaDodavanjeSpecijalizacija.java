...
public class IzborLaborantaDodavanjeSpecijalizacija extends JFrame {
	 ...
	 * Create the frame.
	 */
	public IzborLaborantaDodavanjeSpecijalizacija(String korisnickoImeUlogovanogKorisnika) {
		setTitle("Izbor laboranata");
	...
	private void proveraUnosaKorisnickogImena(String unetoKorisnickoIme, String korisnickoImeUlogovanogKorisnika) {
		...
		if (!KorisniciController.korisnickaImenaLaboranata.contains(unetoKorisnickoIme)){
					
			JOptionPane.showMessageDialog(new JFrame(), "Ovo korisnicko ime laboranta ne postoji!", "Greska", JOptionPane.ERROR_MESSAGE);
		}
		
		else {
			
			setVisible(false);
			DodavanjeSpecijalizacija dodavanjeSpecijalizacija = new DodavanjeSpecijalizacija(korisnickoImeUlogovanogKorisnika, unetoKorisnickoIme);
			dodavanjeSpecijalizacija.setVisible(true);
		}
